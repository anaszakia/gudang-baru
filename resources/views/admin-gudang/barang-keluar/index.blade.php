@extends('layouts.app')
@section('title', 'Manajemen Barang Keluar')
...
<div x-data="{ 
    ...
    <div class="bg-white rounded-lg shadow-sm border p-6">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
                ...
                <p class="text-gray-600 mt-1">Kelola transaksi barang keluar</p>
            </div>
            
            <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                {{-- Search Form --}}
                <form method="GET" action="{{ route('admin-gudang.barang-keluar.index') }}" class="flex gap-2">
                    <div class="relative">
                ...
                {{-- Add Button --}}
                <a href="{{ route('admin-gudang.barang-keluar.create') }}" 
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @forelse($barangKeluars as $index => $bk)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $barangKeluars->firstItem() + $index }}</td>
                        <td class="px-6 py-4 text-sm font-mono text-blue-600 font-medium">{{ $bk->kode_barang_keluar }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $bk->penerima }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ \Carbon\Carbon::parse($bk->tanggal_keluar)->format('d M Y') }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">Rp {{ number_format($bk->total_harga ?? $bk->detailBarangKeluars->sum('subtotal'), 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-sm">
                            @if($bk->status === 'draft')
                                <span class="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded">Draft</span>
                            @else
                                <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">Finalized</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">
                            @if($bk->payment === 'cash')
                                <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded">Cash</span>
                            @elseif($bk->payment === 'transfer')
                                <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">Transfer</span>
                            @elseif($bk->payment === 'piutang')
                                <span class="bg-red-100 text-red-800 text-xs font-medium px-2.5 py-0.5 rounded">Piutang</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                {{-- Detail Button --}}
                                <button @click="openModal('detail', {{ $bk->id }})" 
                                ...
                                <a href="{{ route('admin-gudang.barang-keluar.invoice', $bk->id) }}" target="_blank"
                                ...
                                {{-- Edit Button --}}
                                <button @click="openModal('edit', {{ $bk->id }})" 
                                ...
                                {{-- Delete Button --}}
                                <form action="{{ route('admin-gudang.barang-keluar.destroy', $bk) }}" method="POST" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="confirmDelete(this.closest('form'), '{{ $bk->kode_barang_keluar }}')" 
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            ...
                            <p class="text-gray-400">Mulai dengan menambahkan barang keluar pertama Anda</p>
                        </td>
                    </tr>
                @endforelse
                </tbody>
    ...
    {{-- DETAIL MODAL untuk setiap barang keluar --}}
    @foreach($barangKeluars as $bk)
        <div x-show="detailModal === {{ $bk->id }}" x-cloak 
            ...
            <div @click.away="closeModal('detail')" 
                ...
                <div class="flex justify-between items-center p-6 border-b">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900">Detail Produk - {{ $bk->kode_barang_keluar }}</h3>
                        <p class="text-sm text-gray-500 mt-1">{{ $bk->penerima }} • {{ \Carbon\Carbon::parse($bk->tanggal_keluar)->format('d M Y') }}</p>
                    </div>
                ...
                <div class="p-6 overflow-y-auto max-h-[calc(90vh-160px)]">
                    @if($bk->detailBarangKeluars->count() > 0)
                        <div class="border rounded-lg overflow-hidden">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($bk->detailBarangKeluars as $index => $detail)
                                        <tr class="hover:bg-gray-50">
                        ...
                        <div class="mt-6 pt-4 border-t">
                            <div class="flex justify-between items-center">
                                <div class="text-sm text-gray-500">
                                    Total {{ $bk->detailBarangKeluars->count() }} produk
                                </div>
                                <div class="text-right">
                                    <div class="text-sm text-gray-500">Total Nilai Transaksi</div>
                                    <div class="text-2xl font-bold text-gray-900">Rp {{ number_format($bk->detailBarangKeluars->sum('subtotal'), 0, ',', '.') }}</div>
                                </div>
                            </div>
                        </div>
                    @else
                ...
                <div class="flex justify-end gap-3 p-6 border-t bg-gray-50">
                    <a href="{{ route('admin-gudang.barang-keluar.detail', $bk->id) }}" 
    ...
    {{-- EDIT MODAL untuk setiap barang keluar --}}
    @foreach($barangKeluars as $bk)
        <div x-show="editModal === {{ $bk->id }}" x-cloak 
             class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div @click.away="closeModal('edit')" 
                ...
                {{-- Modal Content --}}
                <form method="POST" action="{{ route('admin-gudang.barang-keluar.update', $bk) }}" class="p-6">
                    @csrf @method('PUT')
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Kode Barang Keluar</label>
                            <input type="text" value="{{ $bk->kode_barang_keluar }}" 
                        ...
                        <div>
                            ...
                            <input type="text" name="penerima" value="{{ $bk->penerima }}" 
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_keluar" value="{{ $bk->tanggal_keluar }}" 
                                   class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500" 
                                   required>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="payment" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500" required>
                                <option value="cash" {{ $bk->payment === 'cash' ? 'selected' : '' }}>Cash</option>
                                <option value="transfer" {{ $bk->payment === 'transfer' ? 'selected' : '' }}>Transfer</option>
                                <option value="piutang" {{ $bk->payment === 'piutang' ? 'selected' : '' }}>Piutang</option>
                            </select>
                        </div>
                    </div>
    ...
    <div id="filterModal" class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 p-4 hidden">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            ...
            {{-- Modal Header --}}
            <div class="flex justify-between items-center p-6 border-b">
                <h3 class="text-lg font-semibold text-gray-900">Filter Laporan Barang Keluar</h3>
            ...
            <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div class="text-sm text-gray-500">
                            Data yang akan diexport adalah transaksi finalized pada rentang tanggal yang dipilih.
                        </div>
                    </div>                {{-- Modal Footer --}}
                <div class="flex justify-end gap-3 pt-6 mt-6 border-t">
    ...
    {{-- PAGINATION --}}
    @if(isset($barangKeluars) && method_exists($barangKeluars, 'links'))
...
<script>
    ...
    function exportData() {
        ...
        exportForm.action = '{{ route('admin-gudang.barang-keluar.export-excel') }}';
