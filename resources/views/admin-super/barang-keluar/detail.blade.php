...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Detail Transaksi Barang Keluar</h1>
                <p class="text-sm text-gray-500 mt-1">Tambahkan produk ke transaksi {{ $barangKeluar->kode_barang_keluar }}</p>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('admin-super.barang-keluar.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2.5 rounded-lg transition-colors font-medium flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                    </svg>
                    Batal
                </a>
                <a href="{{ route('admin-super.barang-keluar.finalize', $barangKeluar->id) }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2.5 rounded-lg transition-colors font-medium flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                <div class="font-medium text-gray-900">{{ $barangKeluar->kode_barang_keluar }}</div>
            </div>
            <div class="bg-gray-50 p-4 rounded-lg">
                <div class="text-sm text-gray-500">Tanggal Keluar</div>
                <div class="font-medium text-gray-900">{{ \Carbon\Carbon::parse($barangKeluar->tanggal_keluar)->format('d M Y') }}</div>
            </div>
            <div class="bg-gray-50 p-4 rounded-lg">
                <div class="text-sm text-gray-500">Penerima</div>
                <div class="font-medium text-gray-900">{{ $barangKeluar->penerima }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form method="POST" action="{{ route('admin-super.barang-keluar.store-detail', $barangKeluar->id) }}" id="addProductForm" class="space-y-4">
                @csrf
                    ...
                    <div>
                        ...
                        <select id="produk_id" name="produk_id" 
                            ...
                            @foreach($produks as $produk)
                                <option value="{{ $produk->id }}">{{ $produk->nama }} - {{ $produk->kode }} (Stok: {{ $produk->stok }})</option>
                            @endforeach
                        </select>
                        @error('produk_id')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:col-span-2">
            ...
            @if($barangKeluar->detailBarangKeluars->isEmpty())
                            ...
                            @foreach($barangKeluar->detailBarangKeluars as $index => $detail)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <form action="{{ route('admin-super.barang-keluar.destroy-detail', $detail->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="mt-6 border-t border-gray-200 pt-4">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <span class="ml-2 text-lg font-semibold text-gray-900">{{ $barangKeluar->detailBarangKeluars->count() }}</span>
                        </div>
                        <div>
                            <span class="text-sm font-medium text-gray-500">Total Nilai:</span>
                            <span class="ml-2 text-lg font-semibold text-gray-900">Rp {{ number_format($barangKeluar->detailBarangKeluars->sum('subtotal'), 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            @endif
...
<script>
    ...
    function searchProductByCode() {
        ...
        if ('{{ $produk->kode }}'.toLowerCase() === kode.toLowerCase()) {
            infoElement.textContent = 'Produk: {{ $produk->nama }} - Rp. {{ number_format($produk->harga, 0, ',', '.') }} (Stok: {{ $produk->stok }})';
        ...
        if (!found) {
            errorElement.textContent = 'Kode produk tidak ditemukan atau stok habis';
