...
<div x-data="{ 
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ auth()->user()->role === 'admin-super' ? 'Manajemen Produk' : 'Daftar Produk' }}</h1>
                <p class="text-sm text-gray-500 mt-1">{{ auth()->user()->role === 'admin-super' ? 'Kelola produk sistem' : 'Informasi produk sistem' }}</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                {{-- Search Form --}}
                <form method="GET" action="{{ auth()->user()->role === 'admin-super' ? route('admin-super.produks.index') : route('user.produks.index') }}" class="flex gap-2">
                    <div class="relative">
                ...
                @forelse($produks as $produk)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            ...
                            {{-- Delete Button --}}
                            <form action="{{ route('admin-super.produks.destroy', $produk) }}" method="POST" class="inline delete-form">
                                @csrf @method('DELETE')
                    ...
                    <div x-show="editModals[{{ $produk->id }}]" x-cloak 
                        ...
                        <div @click.away="toggleModal('edit', {{ $produk->id }})" 
                            ...
                            <div class="px-6 py-4">
                                <form method="POST" action="{{ route('admin-super.produks.update', $produk) }}" class="space-y-4">
                                    @csrf @method('PUT')
                                    <div class="grid grid-cols-2 gap-4">
                                        {{-- Kode (readonly) --}}
                                        <div>
    ...
    <div x-show="openCreate" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div @click.away="toggleModal('create')" class="bg-white rounded-xl shadow-xl w-full max-w-lg">
            ...
            <div class="px-6 py-4">
                <form method="POST" action="{{ route('admin-super.produks.store') }}" id="createForm" class="space-y-4">
                    @csrf
